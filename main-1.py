...
async def main() -> None:
    # نفتح بورت keep-alive **أولاً** قبل أي شيء آخر، عشان Render يكتشف
    # البورت من أول لحظة إقلاع ولا يحتاج يعيد تشغيل الخدمة من تلقاء نفسه
    # منتصف التشغيل (وهو التصرف اللي كان بيسبب تعارض getUpdates مؤقت).
